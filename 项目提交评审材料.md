...
| 源代码仓库 | 主仓库 fork：`https://github.com/yufangjie1643/open-design`；上游仓库：`https://github.com/nexu-io/open-design` |
| 提交分支 | `codex/ecommerce-video-workbench` |
| 最后提交记录 | `1fcf3aad3d5b648dc9ae3da086b9a14e40931cc3`，提交说明：`chore: update ecommerce video workbench` |
| 在线 Demo 链接 | 待补充；本地体验地址为 `http://127.0.0.1:17573` |
| APP 安装包 | 待补充；若不提供安装包，可提供本地启动说明和录屏替代 |
| Agent skill 包 | `skills/commerce-video-methodology-extractor/` |
| 演示视频链接 | 待补充，建议 3-8 分钟公开可访问链接 |
| README / 运行说明 | 本文第 8 节；仓库已有 `README.md`、`QUICKSTART.md` |

## 2. 团队成员与分工

| 成员 | 学校 / 专业 | 角色 | 分工 |
|---|---|---|---|
| 余方杰 | 待补充 | 全栈开发 / AI Agent 编排 / 工程联调 | 前端工作台、后端 commerce-video workflow、素材库与生成接口、CLI、测试、提报材料整理 |
| 待补充 | 待补充 | 待补充 | 可填写：产品设计、演示视频、测试素材、部署、模型调参 |
| 待补充 | 待补充 | 待补充 | 可填写：数据看板、素材标注、脚本模板、答辩材料 |
...
1. 商品素材与参考视频入库  
   支持商品图片 / 视频、参考素材、公开视频搜索结果进入素材库；后端可记录来源、类目、摘要、标签、文件路径和处理状态。
...
| `apps/daemon/src/routes/asset-library.ts` | 商品素材、带货视频、质量视频、Embedding、处理任务等素材库 API |
...
| 能力 | 说明 |
|---|---|
| 商品素材库 | 商品图、商品视频、参考素材入库，支持项目上下文绑定 |
...
erDiagram
  ...
  asset_library_products ||--o{ asset_library_project_context : referenced_by
  ...
  asset_library_products ||--o{ asset_library_jobs : processed_by
...
1. 0:00-0:30 项目价值：商家输入商品素材，系统生成可控带货短视频。
2. 0:30-1:10 展示素材库：商品素材、参考视频、切片、标签、检索。
3. 1:10-2:00 展示方法论 / 剧本：选择模板或由 Agent 生成 Hook、卖点、口播、CTA。
4. 2:00-3:00 展示分镜编辑器：镜头顺序、时长、素材切片、台词、单镜重生成。
5. 3:00-3:50 展示一键成片与任务进度：创建任务、等待进度、失败兜底。
6. 3:50-4:30 展示预览导出：成片路径、导出 manifest。
7. 4:30-5:00 展示数据看板：生成因子、转化 lift、下一轮生成建议。
...
- [ ] 补齐团队名称、成员学校、专业、角色。
- [ ] 补齐在线 Demo 链接；如需要登录，提供体验账号或录屏替代。
- [ ] 补齐演示视频链接，建议公开可访问，时长 3-8 分钟。
- [ ] 确认源代码仓库和分支已 push，提交记录与材料一致。
- [ ] 确认 README 或本文包含依赖环境、启动步骤、目录结构和配置说明。
- [ ] 截图关键页面：素材库、分镜编辑器、生成进度、预览导出、数据看板。
- [ ] 删除或轮换任何已经写入草稿、截图、日志、录屏、仓库的 API Key / Endpoint 私密信息。
- [ ] 运行最小验证命令，记录通过结果或已知限制。
