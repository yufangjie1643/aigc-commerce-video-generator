      ...
      od: {
        ...
        inputs: [
          ...
          { name: "audience", type: "string", required: true, default: "decision makers", label: "Audience" },
          {
            ...
            type: "select",
            required: true,
            options: ["5-10 pages", "10-15 pages", "15-20 pages"],
            default: "10-15 pages",
            label: "Pages"
          },
          {
...
test("[P1] home hero example presets update the composer input for prototype and live artifact", async ({ page }) => {
  ...
  await page.locator('[data-testid="home-hero-plugin-preset"][data-plugin-id="example-web-prototype"]').click();
  await expect(input).toHaveText(
  ...
  await page
    .locator(
      '[data-testid="home-hero-plugin-preset"][data-plugin-id="image-template-notion-team-dashboard-live-artifact"]'
    )
    .click();
...
test("[P1] home hero deck example preset updates the composer input", async ({ page }) => {
  ...
  await expect(page.getByTestId("home-hero-plugin-presets")).toBeVisible();
  await page.locator('[data-testid="home-hero-plugin-preset"][data-plugin-id="example-simple-deck"]').click();
  await expect(input).toHaveText(
...
test("[P1] after clearing one mode, selecting another example updates the composer without leaking prior mode state", async ({
  page
}) => {
  await gotoEntryHome(page);
...
test("[P1] selecting another example updates the composer input", async ({ page }) => {
  ...
  await page.locator('[data-testid="home-hero-plugin-preset"][data-plugin-id="example-live-artifact"]').click();
  await expect(input).toHaveText(
