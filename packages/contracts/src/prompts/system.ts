...
5. To make an imported video usable by scripts/creation, run processing and wait for the job:
\`"$OD_NODE_BIN" "$OD_BIN" assets commerce-videos process <assetId> --wait --json\`
Use explicit slicing when the user asks for slice features:
\`"$OD_NODE_BIN" "$OD_BIN" assets commerce-videos slice <assetId> --wait --json\`
Then inspect slices:
\`"$OD_NODE_BIN" "$OD_BIN" assets commerce-videos slices <assetId> --json\`
6. For vectorization, first check/set embedding config with \`assets embedding get|set|test\`, then run:
\`"$OD_NODE_BIN" "$OD_BIN" assets commerce-videos embed <assetId> --include-slices --wait --json\`
If the job reports \`needs_embedding_config\`, tell the user that the asset and slices are available but vectors need a configured provider key.
7. For batch analysis of existing commerce-video assets, use:
\`"$OD_NODE_BIN" "$OD_BIN" assets commerce-videos batch-process --query "<filter>" --wait --json\`
or pass explicit ids with \`--ids "<id1>,<id2>"\`. This performs download/probe/slice/embedding in sequence and exposes detailed progress through the returned job.
8. To summarize reusable爆款方法论 through the agent, first ensure relevant videos are processed, then fetch structured context:
\`"$OD_NODE_BIN" "$OD_BIN" assets commerce-videos methodology-summary --query "<filter>" --json\`
Use the returned prompt/context together with \`video-storyboard-analysis\` and \`video-generation-pipeline\` to write the methodology. Do not fake the summary without reading the asset-library context.
9. \`assets jobs get <jobId> --json\` and \`assets jobs wait <jobId> --json\` are the agent-facing job status APIs. Read \`job.error.code\` before retrying.
10. \`assets commerce-videos import-search\` is a legacy bulk-import command that writes every returned search result. Do not use it unless the user explicitly asks to import every search result without agent filtering.
...
export function composeSystemPrompt({
  ...
  parts.push(FOLLOW_UP_QUESTION_FORM_DIRECTIVE);
  parts.push("\n\n---\n\n");
...
const FOLLOW_UP_QUESTION_FORM_DIRECTIVE = `# Follow-up choice UI
...
Use a single renderable \`<question-form>\` block for follow-up choices, then stop your turn. Use \`type: "radio"\` or \`type: "select"\`, 2-4 options, a recommended/default option first, and concise option labels. Do not also list the same options as markdown before or after the form.
