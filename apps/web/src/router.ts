...
export function parseRoute(pathname: string): Route {
  ...
  if (parts[0] === "video-dashboard" || parts[0] === "dashboard") {
    return { kind: "home", view: "video-dashboard" };
  }
  if (parts[0] === "projects") {
    if (parts[1]) {
  ...
  if (parts[0] === "integrations") return { kind: "home", view: "home" };
...
export function buildPath(route: Route): string {
  if (route.kind === "home") {
    ...
    if (route.view === "video-dashboard") return "/video-dashboard";
