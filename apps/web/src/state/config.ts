...
import { DEFAULT_FAILURE_SOUND_ID, DEFAULT_SUCCESS_SOUND_ID } from "../utils/notifications";
...
function normalizeNotifications(input: Partial<NotificationsConfig> | undefined): NotificationsConfig {
...
function hasRecoverableLocalMediaProviderFields(entry: MediaProviderCredentials | null | undefined): boolean {
  return Boolean(
    entry?.apiKey?.trim() ||
    entry?.baseUrl?.trim() ||
    entry?.model?.trim() ||
...
function isMarkerOnlyMediaProviderEntry(entry: MediaProviderCredentials | null | undefined): boolean {
  return isStoredMediaProviderEntryPresent(entry) && !hasRecoverableLocalMediaProviderFields(entry);
}

export function isStoredMediaProviderEntryPresent(entry: MediaProviderCredentials | null | undefined): boolean {
  return Boolean(
    entry?.apiKey?.trim() ||
    entry?.baseUrl?.trim() ||
    entry?.model?.trim() ||
...
export function isStoredMediaProviderEntryEmpty(entry: MediaProviderCredentials | null | undefined): boolean {
...
export function buildMediaProvidersForDaemonSave(
  ...
  for (const [providerId, currentEntry] of Object.entries(currentProviders ?? {})) {
    ...
    const imageUnderstandingModel =
      currentEntry?.imageUnderstandingModel?.trim() || daemonEntry?.imageUnderstandingModel?.trim() || "";
    const audioUnderstandingModel =
      currentEntry?.audioUnderstandingModel?.trim() || daemonEntry?.audioUnderstandingModel?.trim() || "";
    const videoUnderstandingModel =
      currentEntry?.videoUnderstandingModel?.trim() || daemonEntry?.videoUnderstandingModel?.trim() || "";
    if (
      !apiKey &&
      !preserveApiKey &&
      !explicitBaseUrl &&
...
export async function syncComposioConfigToDaemon(config: AppConfig["composio"] | undefined): Promise<boolean> {
...
const DAEMON_OWNED_KEYS = new Set<keyof AppConfig>(["installationId", "telemetry", "privacyDecisionAt"]);
...
export function mergeDaemonConfig(localConfig: AppConfig, daemonConfig: AppConfigPrefs | null): AppConfig {
...
export function mergeDaemonMediaProviders(
  ...
  for (const [providerId, daemonEntry] of Object.entries(daemonProviders ?? {})) {
    ...
    const preserveLocalPendingEdit = Boolean(
      options?.preserveLocalProviderIds?.has(providerId) && hasRecoverableLocalMediaProviderFields(localEntry)
    );
    mediaProviders[providerId] = preserveLocalPendingEdit ? { ...daemonEntry, ...localEntry } : { ...daemonEntry };
  }
...
export function hasAnyConfiguredProvider(providers: Record<string, MediaProviderCredentials> | undefined): boolean {
  if (!providers) return false;
  return Object.values(providers).some((entry) => isStoredMediaProviderEntryPresent(entry));
}
...
export async function syncMediaProvidersToDaemon(
  ...
  if (!providers) return;
  try {
    const payload = buildMediaProvidersForDaemonSave(providers, options?.daemonProviders, { force: options?.force });
    const response = await fetch("/api/media/config", {
      method: "PUT",
      headers: { "content-type": "application/json" },
      body: JSON.stringify(payload)
...
export async function syncConfigToDaemon(config: AppConfig, options?: { throwOnError?: boolean }): Promise<void> {
