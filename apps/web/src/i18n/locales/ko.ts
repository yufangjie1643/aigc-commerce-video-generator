...
export const ko: Dict = {
  ...
  "chat.amrCard.switchBody":
    "Open Design 공식 AMR 모델 서비스로 전환하세요 — API 키 설정이 필요 없습니다. 로그인・인증・충전이 완료되면 이 작업이 자동으로 재시도됩니다.",
  ...
  "settings.onboardingMemoryBody":
    "환경 설정, 프로젝트 정보, 반복 규칙을 추가하여 이후 채팅에서 올바른 컨텍스트를 이어가도록 하세요.",
  "settings.onboardingSystemsTitle": "디자인 시스템 가져오기",
  "settings.onboardingSystemsBody":
    "브랜드 시스템을 선택하거나 생성하여 생성된 작업물이 실제 색상, 타이포그래피, 제품 언어를 따르도록 하세요.",
  "settings.onboardingExecutionTitle": "생성 방식 선택",
  "settings.onboardingExecutionBody":
    "원클릭 설정과 즉시 사용 가능한 기본값을 갖춘 공식 CLI입니다. 하나의 키로 더 나은 가격의 다양한 모델 중에서 선택하세요.",
  ...
  "settings.onboardingNewsletterBody":
    "제품 업데이트, 새 템플릿, 디자인 시스템, 앰배서더 활동을 메일로 받아보세요. 선택 사항이며 건너뛸 수 있습니다.",
  ...
  "settings.byokNoFileToolsNotice":
    "BYOK는 프로젝트 파일을 읽거나 쓰거나 편집할 수 없습니다. 코드 변경이 필요한 경우 Local CLI를 사용하세요.",
  ...
  "settings.testInvalidModelId":
    "모델 ID '{model}'이(가) 잘못되었습니다. 사용자 지정 ID는 문자 또는 숫자로 시작하고 공백을 포함할 수 없습니다.",
  ...
  "settings.fetchModelsUnsupportedAzure":
    "Azure OpenAI는 배포 이름을 사용하므로 여기에서는 모델 검색을 사용할 수 없습니다.",
  "settings.fetchModelsUnsupportedOllama":
    "Ollama Cloud 모델 검색은 아직 사용할 수 없습니다. 모델을 선택하거나 입력하세요.",
  ...
  "settings.byokVideoI2vHint":
    "이미지→비디오 모델: 참조 이미지가 필요합니다(지정하지 않으면 프로젝트의 최신 이미지를 자동 사용)",
  ...
  "settings.apiHint":
    "요청은 로컬 daemon 프록시를 통해 설정한 Base URL로 전송됩니다. 키는 이 브라우저에만 저장되며 제공자 요청과 함께 전송됩니다.",
  ...
  "settings.cliEnvHint":
    "CLI 트래픽을 자체 프록시를 통해 라우팅하거나 바이너리를 비표준 위치에 설치한 경우에만 사용하세요. 비밀 정보는 로컬 앱 설정에 저장되며 선택한 CLI만 접근할 수 있습니다.",
  ...
  "settings.mediaProviderClearConfirm":
    "저장된 {name} 설정을 삭제하시겠습니까? {name}을(를) 사용하려면 다시 입력해야 합니다.",
  ...
  "settings.mediaProviderLoadError":
    "로컬 데몬에서 미디어 제공자 설정을 불러오지 못했습니다. 지금은 브라우저에 저장된 설정을 사용합니다.",
  "settings.mediaProviderComingSoonHint":
    "로드맵에서 이를 추적하고 있습니다. 데몬이 아직 클라이언트를 제공하지 않으므로 구성할 항목이 없습니다.",
  ...
  "settings.privacyConsentLead":
    "Open Design은 제품 개선을 위해 사용 데이터를 팀과 공유할 수 있습니다. 여기에는 다음이 포함됩니다:",
  "settings.privacyConsentFooter":
    "설정 → 개인정보 보호에서 언제든지 이 두 가지를 변경할 수 있습니다. 생성된 아티팩트 파일의 내용은 절대 업로드하지 않습니다.",
  ...
  "settings.privacyConsentBannerFooter":
    "데이터 공유는 기본적으로 켜져 있습니다. 설정 → 개인정보 보호에서 언제든지 끌 수 있습니다. 생성된 아티팩트 파일의 내용은 절대 업로드하지 않습니다.",
  "settings.privacyConsentPolicyLink": "개인정보 처리방침 읽기",
  "settings.privacyMetrics": "익명 측정 지표",
  "settings.privacyMetricsHint":
    "실행 횟수, Token 사용량, 오류율, 소요 시간. 프롬프트나 프로젝트 데이터는 포함되지 않습니다.",
  "settings.privacyContent": "대화 및 도구 콘텐츠",
  "settings.privacyContentHint":
    "프롬프트, 어시스턴트 응답, 도구 입력 및 도구 출력(전송 전 잘림). API key, Token, JWT, 이메일, IP, 신용카드 번호는 전송 전 자동으로 제거됩니다.",
  ...
  "settings.privacyDataDeletionHint":
    "익명 ID를 교체하고 전송을 중단합니다. 기존 추적 데이터는 보관 정책에 따라 만료됩니다.",
  "settings.about": "정보",
  ...
  "settings.mcpDaemonError":
    "설치 경로를 확인하기 위해 로컬 데몬에 연결하지 못했습니다 ({error}). Open Design이 실행 중인지 확인한 후 이 패널을 다시 열어 주세요.",
  "settings.mcpBuildDaemon": "먼저 데몬을 빌드하세요.",
  "settings.mcpNodeMissing": "Node 바이너리가 없습니다.",
  "settings.mcpBuildHint":
    "apps/daemon/dist/cli.js가 없습니다. `pnpm --filter @open-design/daemon build`를 실행한 후 새로고침하세요.",
  ...
  "settings.mcpInstructionCursor":
    '"Install in Cursor"를 클릭하여 승인 대화상자와 함께 설치하거나, 이 JSON을 {path}에 병합하세요.',
  "settings.mcpDeeplinkInstallCursor": "Cursor에 설치",
  "settings.mcpMethodJson": "JSON 설정",
  "settings.mcpInstructionCopilot":
    'Command Palette({shortcut})를 열고 "MCP: Open User Configuration"을 실행한 후 이 JSON을 병합하세요. 도구가 표시되려면 Copilot Chat이 Agent 모드여야 합니다.',
  "settings.mcpInstructionAntigravity":
    'Antigravity에서: Agent 패널 "..." 메뉴 → MCP Servers → Manage MCP Servers → View raw config. 이 JSON을 병합하세요.',
  "settings.mcpInstructionZed":
    'Zed 설정({shortcut})을 열고 이것을 최상위 객체에 병합하세요. Zed는 "mcpServers"가 아닌 "context_servers"를 사용합니다.',
  "settings.mcpInstructionWindsurf":
    "{path}을(를) 열거나(또는 Cascade의 MCPs 아이콘 → Configure 사용) 다음을 병합하세요:",
  ...
  "settings.mcpCodexOneClickUnavailable":
    "PATH에서 Codex CLI를 찾을 수 없습니다 — Codex를 설치하거나 스니펫을 수동으로 붙여넣으세요.",
  ...
  "settings.mcpRestartDetail":
    "대부분의 편집기는 시작할 때만 MCP 서버를 불러옵니다. Cursor / VS Code / Antigravity / Windsurf에서는 전체 재시작 대신 command palette에서 `Developer: Reload Window`를 실행할 수 있습니다. Zed와 Claude Code는 종료 후 다시 열어야 합니다.",
  ...
  "settings.mcpCapabilityPull":
    "한 번의 호출로 디자인 번들을 가져옵니다: 진입 파일과 그것이 참조하는 모든 CSS 변수, 컴포넌트, 폰트까지 함께 가져옵니다.",
  "settings.mcpCapabilityDefault":
    'Open Design에서 열려 있는 프로젝트와 파일을 기본값으로 사용하므로, 어떤 디자인인지 다시 말하지 않고도 "이걸 내 앱에 만들어 줘"라고 말할 수 있습니다.',
  "settings.mcpRunningNote":
    "MCP 도구 호출이 성공하려면 Open Design이 실행 중이어야 합니다. Open Design을 열기 전에 코딩 Agent를 시작했다면, 실행 중인 데몬에 연결할 수 있도록 Agent를 다시 시작하세요.",
  ...
  "homeHero.footer.autoDesignSystemSummary":
    "현재 프롬프트에 가장 알맞은 디자인 시스템과 비주얼 스타일을 자동으로 선택합니다.",
  ...
  "workingDirPicker.unavailable":
    "이 빌드에서는 폴더 선택기를 사용할 수 없습니다. 폴더를 선택하려면 데스크톱 앱을 실행하세요.",
  ...
  "handoff.promptReadFiles":
    "먼저 이 디렉터리에 진입하거나 읽으세요. DESIGN.md, README, 기존 HTML/CSS/JS, 에셋, 그리고 있다면 package.json을 우선시하세요.",
  "handoff.promptKeepDesign":
    "현재의 시각적 디자인, 레이아웃, 인터랙션, 에셋을 유지하세요. 계획 단계에서 멈추지 마세요.",
  "handoff.promptProduceCode":
    "실제로 실행 가능한 {framework} 코드를 생성하거나 수정하세요. 프로젝트에 이미 더 명확한 스택이 있다면 충돌을 알리고 결과물이 실행 가능한 상태를 유지하세요.",
  ...
  "pluginsView.lede":
    "설치된 워크플로를 둘러보고, 레지스트리 항목을 찾아보고, 소스를 관리하고, 팀 배포용 플러그인을 준비하세요.",
  ...
  "pluginsView.installedEmpty":
    "아직 설치된 사용자 플러그인이 없습니다. 만들기 / 가져오기를 사용하거나 사용 가능 항목을 설치하세요.",
  ...
  "pluginsView.availableEmptyInstalled":
    "아직 사용 가능한 항목이 없습니다. 설치된 카탈로그 항목은 사용 가능에서 제거됩니다. 다시 사용 가능하게 하려면 항목을 제거하세요.",
  ...
  "pluginsView.teamBody":
    "이 영역은 엔터프라이즈 및 팀 카탈로그, 비공개 신뢰 정책, 공유 플러그인 수명 주기 제어를 위해 예약되어 있습니다.",
  ...
  "integrations.lede":
    "외부 시스템을 연결하고, MCP 도구를 Agent 루프에 가져오고, 다른 IDE, 스크립트, 자동화에서 Open Design을 사용하세요.",
  ...
  "integrations.skillsBody":
    "Skill 수준의 통합 관리는 다른 브랜치에서 이전되고 있습니다. 이 탭은 MCP, Connectors, 향후 Skills 설정이 동일한 Integration 경로에 위치하도록 예약되어 있습니다.",
  ...
  "mcpClient.emptyBody":
    '"서버 추가"를 클릭해 시작하세요 — 템플릿(Higgsfield OpenClaw, Pollinations, Allyson, Imagician, EdgeOne Pages, GitHub, Filesystem…)을 선택하거나 사용자 지정 stdio / HTTP 서버를 설정하세요.',
  ...
  "useEverywhere.section.cli.intro":
    "`od` 바이너리는 데몬과 함께 제공되며, Claude Code / Codex가 생성을 실행할 때 사용하는 것과 동일한 바이너리입니다. 대부분의 하위 명령은 로컬 데몬에 POST하는 얇은 클라이언트이므로, `pnpm tools-dev`로 실행했든 패키징된 앱으로 실행했든 동일하게 작동합니다.",
  "useEverywhere.section.cli.bullet1": "`od` (인수 없음) — 데몬을 부팅하고 웹 UI를 엽니다.",
  "useEverywhere.section.cli.bullet2":
    "`od media generate ...` — 통합 미디어 프로토콜을 통해 이미지 / 비디오 / 오디오 바이트를 생성합니다.",
  "useEverywhere.section.cli.bullet3": "`od run ...` — 프롬프트 + skill로부터 프로젝트 실행을 시작합니다.",
  "useEverywhere.section.cli.bullet4":
    "`od plugin install <source>` / `od plugin apply <id>` — 커뮤니티 플러그인을 설치하고 적용합니다.",
  "useEverywhere.section.cli.bullet5":
    "`od skills list` / `od design-systems list` — 로컬에서 사용 가능한 것을 검사합니다.",
  "useEverywhere.section.cli.bullet6":
    "`od status` / `od doctor` — 데몬 상태를 확인하고 PATH의 Agent CLI를 감지합니다.",
  ...
  "useEverywhere.section.cli.footer":
    "모든 하위 명령은 `--daemon-url http://127.0.0.1:<port>`를 받아 특정 실행 중인 데몬을 대상으로 지정할 수 있습니다 — 테스트용 샌드박스 두 번째 인스턴스를 실행할 때 유용합니다.",
  ...
  "useEverywhere.section.mcp.intro":
    "Open Design에는 Model Context Protocol 서버(`od mcp`)가 포함되어 있어, MCP 지원 클라이언트 — Cursor, Claude Code, Antigravity, VS Code Copilot Chat, openclaw, hermes — 가 수동으로 셸을 거치지 않고도 Open Design 도구(skills 나열, 미리 보기 렌더링, 미디어 생성, 플러그인 실행)를 발견할 수 있습니다. 데몬은 각 주요 클라이언트에 대해 바로 붙여 넣을 수 있는 설치 스니펫을 `GET /api/mcp/install-info`를 통해 게시합니다.",
  "useEverywhere.section.mcp.bullet1": "stdio 전송 — 추가 포트가 없으며, 클라이언트가 `od mcp`를 직접 생성합니다.",
  "useEverywhere.section.mcp.bullet2":
    "사이드카로 실행될 때 로컬 IPC 상태 소켓을 통해 실행 중인 데몬 URL을 자동으로 검색합니다.",
  "useEverywhere.section.mcp.bullet3":
    "일반 설치의 경우 `--daemon-url http://127.0.0.1:<port>`로 대체하여 MCP 프로세스가 항상 실행 중인 데몬을 찾을 수 있도록 합니다.",
  "useEverywhere.section.mcp.bullet4":
    "`OD_DATA_DIR`을 고정하여 생성된 MCP 프로세스가 데몬이 이미 사용 중인 위치와 동일한 곳에 기록하도록 합니다(패키징된 macOS 앱 번들에서 EPERM 방지).",
  ...
  "useEverywhere.section.mcp.footer":
    "Open Design 앱에서 설정 → 통합을 열어 JSON을 직접 편집하는 대신 클라이언트별 설치 명령(Cursor, Claude Code, Antigravity, VS Code)을 복사하세요.",
  ...
  "useEverywhere.section.http.intro":
    "로컬 데몬은 `http://127.0.0.1:7456`에서 HTTP API를 제공합니다(포트 구성 가능). 웹 UI가 호출하는 모든 엔드포인트는 스크립트에서도 마찬가지로 사용할 수 있습니다. 스트리밍 엔드포인트(채팅 턴, 프로젝트 실행)는 `@open-design/contracts`의 계약 타입과 함께 Server-Sent Events를 내보냅니다.",
  ...
  "useEverywhere.section.http.bullet3":
    "`GET /api/projects` 및 `POST /api/projects` — 프로젝트 나열 및 생성 (POST는 프로젝트 + 첫 대화를 반환합니다).",
  "useEverywhere.section.http.bullet4": "`GET /api/projects/:id/chat` — 대화에 대한 Agent 이벤트의 SSE 스트림.",
  "useEverywhere.section.http.bullet5":
    "`POST /api/plugins/:id/apply` — 설치된 플러그인을 바인딩하고 렌더링된 예제 쿼리 + 입력을 가져옵니다.",
  ...
  "useEverywhere.section.http.footer":
    "모든 요청/응답에 대한 순수 TypeScript 타입은 `@open-design/contracts`에 있습니다 — 생성기를 연결하지 않고도 완전한 자동 완성을 위해 스크립트에서 가져오세요.",
  ...
  "useEverywhere.section.skills.intro":
    "Skill은 Claude 호환 `SKILL.md`(YAML 프런트 매터 + 본문)를 포함하는 디렉터리입니다. Open Design은 `od:` 네임스페이스(`mode`, `preview`, `design_system`, `inputs`, …)로 형식을 확장하므로 동일한 아티팩트를 Open Design 내부와 Claude Code, Codex, openclaw, hermes 같은 일반 Agent 양쪽에서 사용할 수 있습니다. 발견은 우선순위 체인을 따르므로 프로젝트가 자체 skills를 재정의할 수 있습니다.",
  "useEverywhere.section.skills.bullet1":
    "발견: `./.claude/skills/` → `./skills/` → `~/.claude/skills/` (프로젝트 우선).",
  "useEverywhere.section.skills.bullet2": "하나의 스킬을 여러 프로젝트에 심볼릭 링크로 연결하여 복사 없이 공유하세요.",
  "useEverywhere.section.skills.bullet3":
    "각 스킬은 커넥터, 아톰, 디자인 시스템 요구 사항, 그리고 갤러리용 `preview` 예시 출력을 선언할 수 있습니다.",
  "useEverywhere.section.skills.bullet4":
    "헤드리스: PATH에 `od`가 있는 Agent는 `od skills list`를 호출한 다음 모든 스킬을 실행할 수 있습니다. 읽기 전용 플로우에서는 데몬이 선택 사항입니다.",
  "useEverywhere.section.skills.bullet5":
    "`pnpm seed:test-projects`는 기본 플러그인 예시와 커뮤니티 플러그인 예시로 동일한 아티팩트 형태를 실행한 다음, 결과로 나온 `index.html` 프로젝트를 재사용 가능한 테스트 데이터로 저장합니다.",
  ...
  "useEverywhere.section.skills.footer":
    "명세: `docs/skills-protocol.md`와 `docs/agent-adapters.md`는 전체 어댑터 표면(Claude Code, Codex, Cursor, MCP 지원 Agent, BYOK API 폴백)과 어댑터별 스킬 주입 전략을 다룹니다.",
  "tasks.kicker": "자동화 워크스페이스",
  "tasks.comingSoon": "출시 예정",
  "tasks.lede":
    "자동화는 프롬프트를 지속적인 작업으로 전환합니다. Orbit가 이를 실행하고, 루틴이 이를 유지하며, 스케줄이 실행 시점을 결정하고, 라이브 아티팩트가 변경된 내용을 보여줍니다.",
  "tasks.newAutomation": "새 자동화",
  "tasks.previewNote":
    "미리 보기 표면 전용. Orbit 설정은 현재 사용 가능하며, 루틴, 스케줄, 라이브 아티팩트 연결은 백엔드 브랜치가 병합되면서 추가될 예정입니다.",
  ...
  "promptTemplates.attributionFooter":
    "공개 프롬프트 라이브러리를 기반으로 합니다. 각 카드는 원작자 페이지로 연결됩니다.",
  ...
  "newproj.targetPlatformsHint":
    "하나 이상 선택하세요. 반응형 웹은 브라우저 중단점만 지원합니다. 네이티브 크로스 플랫폼 버전을 위해 iOS, Android, 태블릿 앱 또는 데스크톱 앱을 추가하세요.",
  ...
  "newproj.includeLandingPageHint":
    "광고, 대기자 명단, 출시 캠페인, 앱 다운로드 또는 제품 설명을 위한 반응형 마케팅 페이지를 추가하세요.",
  "newproj.includeOsWidgets": "OS 위젯 포함",
  "newproj.includeOsWidgetsHint":
    "모바일/태블릿 앱을 위한 플랫폼 네이티브 홈 화면, 잠금 화면 또는 빠른 액세스 위젯을 추가하세요.",
  "newproj.includeOsWidgetsDisabledHint":
    "iOS, Android 또는 태블릿 앱이 대상 플랫폼으로 선택된 경우 사용할 수 있습니다.",
  ...
  "newproj.connectorsEmptyBody":
    "데이터 소스를 연결해 라이브 아티팩트가 자리표시자 대신 실제 데이터를 사용하도록 하세요.",
  ...
  "socialShare.openDesignText":
    "Open Design은 디자인 아티팩트를 생성, 편집, 배포 및 인수인계하기 위한 오픈소스 워크스페이스입니다.",
  ...
  "socialShare.projectText":
    "Open Design으로 제작: {title}. 배포된 아티팩트를 확인하고 저장소에 스타를 남겨주세요: {repo}",
  ...
  "chat.mode.design.summary":
    "페이지, 프로토타입, 라이브 아티팩트, 슬라이드, 이미지, 비디오, HyperFrames, 오디오, 대시보드, 프로젝트 파일 같은 구체적인 결과물을 만들거나 변경합니다.",
  "chat.mode.design.solves":
    "Open Design이 파일을 생성하고 workspace를 업데이트하거나 brief를 멀티모달 deliverable로 바꿔야 할 때 사용합니다.",
  ...
  "browserUse.title": "영감",
  "browserUse.summary":
    "{count}개의 웹 작업으로, 에셋 추출, 디자인 언어, 모션, 시각적 검사, 컴포넌트 구조를 우선합니다.",
  ...
  "browserUse.action.extract_logo.output":
    "헤더, 내비게이션, 클래스 이름, 위치, OG 이미지 및 파비콘 대체에서 추출한 최적의 로고 후보.",
  ...
  "browserUse.action.extract_fonts.output":
    "상위 글꼴 패밀리, 크기, 굵기 및 @font-face 규칙을 typography.json으로 저장.",
  ...
  "browserUse.action.extract_buttons.output":
    "중복 제거된 버튼 스타일 라이브러리를 buttons.html 및 buttons.json으로 저장.",
  "browserUse.action.extract_grid_system.output":
    "그리드 및 플렉스 컨테이너, 방향, 간격, 열, 최대 너비를 layout.json으로 저장.",
  ...
  "browserUse.action.generate_styleguide.output":
    "색상, 타입 스케일, 모서리 반경, 그림자가 포함된 한 페이지 스타일 가이드.",
  ...
  "browserUse.action.snapshot.output":
    "태그, 레이블, href, 좌표가 포함된 최대 120개의 표시되는 인터랙티브 및 텍스트 요소.",
  ...
  "chat.designToolbox.action.auto-match.description":
    "먼저 취향 기준을 정한 다음, 스킬, MCP, 플러그인, 커넥터, 파일을 매칭하세요.",
  ...
  "chat.designToolbox.action.motion.description":
    "현재 HTML 또는 페이지 요소에 등장, 스크롤, 상태 전환, 마이크로 인터랙션 모션을 추가합니다.",
  ...
  "chat.designToolbox.action.motion-polish.description":
    "기존 모션의 타이밍, 이징, 성능, 모션 감소 대체 동작을 검토합니다.",
  ...
  "chat.designToolbox.action.anti-ai-polish.description":
    "템플릿 느낌, AI 그라데이션, 저렴한 카드 스택, 모호한 카피를 제거합니다.",
  ...
  "chat.designToolbox.action.visual-polish.description":
    "시각적 위계, 타이포그래피, 간격, 반응형 동작, 접근성, 전달 품질을 점검합니다.",
  ...
  "chat.designToolbox.action.image-gen.description":
    "현재 페이지를 위한 섹션 레퍼런스, 에셋, 아이콘, 소셜 이미지, 무드보드를 생성합니다.",
  ...
  "chat.designToolbox.action.video-gen.description":
    "디자인을 짧은 비디오, Remotion / Hyperframes 스토리보드, 또는 생성 가능한 프롬프트로 변환합니다.",
  ...
  "chat.designToolbox.prompt.noSkill":
    "특정 skill이 일치하지 않으면, 먼저 사용 가능한 skill, MCP, 플러그인, 커넥터, 디자인 파일에서 최적의 조합을 선택하세요.",
  ...
  "chat.designToolbox.prompt.autoMatchStep1":
    "1. 먼저 “보기 좋은 디자인”이 무엇인지 정의하세요: 대상, 비즈니스 목표, 브랜드 느낌, 스타일 레퍼런스, 정보 밀도, 색상/타이포/모션/에셋 기준, 그리고 피해야 할 AI/템플릿 패턴.",
  "chat.designToolbox.prompt.autoMatchStep2":
    "2. 위의 전체 리소스 인덱스를 검색하고 매칭하세요. 디자인 툴박스 추천만 사용하지 말고, 유용할 때 skill, MCP, 플러그인, 커넥터, 디자인 파일을 결합하세요.",
  "chat.designToolbox.prompt.autoMatchStep3":
    "3. 대상이나 미적 기준이 아직 불분명하면, 한정된 선택지나 폼 형태의 UI로 안내하고 권장 기본값을 포함하세요. 충분히 명확하면 다음 단계를 바로 실행하세요.",
  "chat.designToolbox.prompt.autoMatchStep4":
    "4. 다음 순서로 플로를 완료하세요: 진단 -> 스타일 방향 -> 리소스 선택 -> 구체적 편집/생성 -> 검증. 각 단계에서 이 리소스를 선택한 이유와 다음에 무엇을 확인해야 하는지 설명하세요.",
  "chat.designToolbox.prompt.motion":
    "현재 HTML / 페이지 요소에 고품질 모션을 추가하세요: 가장 유용한 진입, 스크롤, 상태 전환 또는 마이크로 인터랙션 순간을 1-2개 선택하세요. 절제된 표현을 유지하고, transform / opacity를 우선하며, prefers-reduced-motion 폴백을 포함하세요.",
  "chat.designToolbox.prompt.motionPolish":
    "기존 모션의 타이밍, 이징, 성능, 접근성을 검토하고 다듬으세요. 갑작스럽거나 조잡하거나 가독성을 해치는 애니메이션을 제거하고, 필요할 때 더 정교한 타임라인을 사용하세요.",
  "chat.designToolbox.prompt.antiAiPolish":
    "AI 느낌 제거 폴리싱을 한 차례 수행하세요: 틀에 박힌 레이아웃, 조잡한 그라데이션/글로우, 의미 없는 카드 스택, 모호한 카피를 제거하세요. 정보는 유지하고 실제 디자이너가 만든 것처럼 느껴지게 하세요.",
  "chat.designToolbox.prompt.visualPolish":
    "이 디자인을 출시 준비가 될 때까지 다듬으세요: 위계, 타이포그래피, 간격, 반응형 동작, 버튼 상태, 빈/로딩/오류 상태, 접근성을 점검하고, 가장 중요한 문제를 바로 수정하세요.",
  "chat.designToolbox.prompt.imageGen":
    "이 디자인의 다음 비주얼 에셋 계획을 만드세요: 섹션 레퍼런스, 히어로 에셋, 일러스트레이션, 아이콘, 소셜 이미지 또는 무드보드. 먼저 어떤 종류의 이미지가 부족한지 결정한 다음, 실행 가능한 생성 프롬프트 / 파일 계획을 제공하세요.",
  "chat.designToolbox.prompt.videoGen":
    "이 디자인을 영상으로 전환하세요: 숏폼 스토리보드, Hyperframes / Remotion 프레임 구조 또는 Sora/fal에 바로 쓸 수 있는 프롬프트를 만드세요. 기존 HTML / 페이지 콘텐츠가 자연스럽게 샷이 되도록 하는 것을 우선하세요.",
  "chat.designToolbox.prompt.useSkill": "현재 디자인에 {skill}을(를) 사용하세요.",
  "chat.designToolbox.prompt.skillInstruction":
    "먼저 최적의 후속 목표를 결정한 다음, 구체적인 작업을 한 차례 완료하세요. 외부 에셋이나 API에 의존하는 경우, 실행 가능한 폴백을 제공하거나 제가 무엇을 제공해야 하는지 명시하세요.",
  ...
  "chat.designToolbox.prompt.pluginResource":
    "이 플러그인을 현재 디자인 플로의 주요 컨텍스트로 사용하세요. 입력이나 GenUI를 노출하는 경우 필요할 때 선택하도록 안내하고, 대상에 맞지 않으면 더 나은 전역 리소스를 선택하고 이유를 설명하세요.",
  "chat.designToolbox.prompt.mcpResource":
    "다음 디자인 단계에서는 이 MCP 기능을 우선하세요. 외부 에셋, 웹 캡처, 이미지/영상 생성 또는 데이터가 필요할 때, 먼저 이 기능으로 처리할 수 있는지 판단한 다음 실행 가능한 계획을 제시하세요.",
  "chat.designToolbox.prompt.mcpTemplateResource":
    "이 MCP는 구성 가능한 템플릿입니다. 이 디자인 단계에서 가장 유용한 도구라면 먼저 설정을 안내하고, 구성된 대안이 있으면 그 대안으로 계속 진행하세요.",
  "chat.designToolbox.prompt.connectorResource":
    "실제 데이터, 에셋, 계정 콘텐츠 또는 외부 워크플로 접근이 필요할 때 이 커넥터를 사용하세요. 사용하기 전에 무엇을 읽거나 쓸지 설명하고, 적절한 경우 확인을 요청하세요.",
  "chat.designToolbox.prompt.fileResource":
    "이 디자인 파일을 현재 디자인 대상 또는 참조 에셋으로 사용하여 다음 단계를 완료하세요. 다른 파일과 비교해야 하는 경우, 모든 디자인 파일에서 계속 검색하세요.",
  "chat.designToolbox.prompt.resourceIndex":
    "전역 리소스 인덱스: skill({skills}), 플러그인({plugins}), MCP(활성화 {mcpEnabled}개 / 템플릿 {mcpTemplates}개), 커넥터(연결 {connectors}개), 디자인 파일({files}).",
  ...
  "chat.designToolbox.prompt.processRule":
    "워크플로 규칙: 먼저 미적 목표와 제약을 정의한 다음 리소스를 검색하고 매칭하세요. 사용자 선택이 필요할 때는 한정된 선택지 UI를 생성하고 확인 후 계속 진행하세요.",
  ...
  "fileViewer.exportImageDownloadDetails":
    "다른 이름으로 저장 창이 나타나지 않았다면 브라우저 다운로드 폴더에서 {filename}을 확인하세요.",
  ...
  "liveArtifact.refresh.timelineEmpty":
    "이 세션에서 아직 새로 고침 활동이 없습니다. 새로 고침을 실행하여 타임라인을 기록하거나 자동 실행을 기다리세요.",
  ...
  "fileViewer.cloudflareApiTokenScopeHint":
    "API 토큰을 만들 때 Pages Edit를 선택하세요. 사용자 지정 도메인에는 Zone Read / DNS Edit가 필요합니다.",
  ...
  "fileViewer.cloudflareCustomDomainHint":
    "선택 사항: 사용자 지정 도메인을 연결합니다. 비워 두어도 pages.dev 링크가 생성됩니다.",
  ...
  "fileViewer.deployLinkProtected":
    "사이트가 배포되었지만 이 미리보기 링크에는 인증이 필요합니다. Deployment Protection을 끄거나 사용자 지정 도메인을 사용하세요.",
  ...
  "assistant.emptyResponseMessage":
    "제공자가 텍스트나 아티팩트를 반환하지 않고 요청을 종료했습니다. 다른 모델이나 제공자를 사용하거나, 할당량을 확인하거나, 다시 시도하세요.",
  ...
  "pet.idleQuote.caravaggio.text":
    "무엇을 그렸든 누가 그렸든, 모든 작품은 실제 삶에서 만들어지고 그려지지 않는 한 하찮은 것이고 유치한 장난에 불과하다.",
  ...
  "pet.imageHintIdle":
    "PNG / JPG / WebP / GIF / SVG 지원. 스프라이트시트? 가로 스트립을 올리고 프레임 수를 설정하세요.",
  ...
  "pet.atlasPickerHint":
    "Codex 펫은 9개의 애니메이션 행을 가집니다. 기본값은 전체 아틀라스를 유지해 호버, 드래그 방향, 장시간 대기에 따라 행이 바뀝니다. 한 행으로 고정할 수도 있습니다.",
  ...
  "pet.atlasActiveHint":
    "애니메이션 아틀라스 활성 — 상호작용(호버, 드래그, 대기)에 따라 펫이 행을 자동으로 전환합니다.",
  ...
  "settings.projectLocationsDescription":
    "여러 Open Design 프로젝트 폴더를 포함할 수 있는 작업 베이스를 추가합니다. 새 프로젝트는 선택한 베이스 안의 폴더로 저장됩니다.",
  "settings.projectLocationsSaveError":
    "프로젝트 위치를 저장할 수 없습니다. 각 경로가 접근 가능한 폴더인지 확인하세요.",
  ...
  "settings.connectorsClearConfirmBody":
    "키를 삭제하면 이 워크스페이스에 연결된 모든 Composio 커넥터가 해제됩니다. 연결된 계정, OAuth 권한, 도구 접근 권한이 모두 제거됩니다.",
  ...
  "settings.connectorsClearFinalBody":
    "되돌릴 수 없습니다. 새 키를 붙여 넣은 뒤에는 각 통합을 처음부터 다시 연결해야 합니다.",
  ...
  "settings.connectorsHelpSaved":
    "키가 아래 카탈로그를 열며 로컬 daemon에 유지됩니다. 교체하려면 새 키를 붙여넣고, 제거하려면 지우세요.",
  "settings.connectorsHelpUnsaved":
    "저장되지 않은 변경사항 — 「키 저장」을 눌러 이 자격 정보를 로컬 데몬에 저장하고 아래 카탈로그를 해제하세요.",
  "settings.connectorsHelpEmpty":
    "아래 카탈로그를 열려면 키를 추가하세요. 키는 daemon에 로컬로 저장되며 환경 변수로 전송되지 않습니다.",
  ...
  "settings.customInstructionsHint":
    "모든 채팅에서 Open Design이 따라야 할 고정 지침입니다. 사실, 선호 사항, 프로젝트 컨텍스트를 메모리로 저장하세요.",
  "settings.customInstructionsPlaceholder":
    '예: "항상 TypeScript를 사용하세요. 함수형 컴포넌트를 선호합니다. 응답은 간결하게 유지하세요."',
  "settings.memoryDescription":
    "취향과 컨텍스트에 관한 사실을 대화에서 자동으로 추출하여 Markdown 파일로 저장하고, 이후 대화에 자동 주입합니다.",
  ...
  "settings.memoryDisabledBanner":
    "메모리가 현재 꺼져 있습니다. 기존 사실은 디스크에 유지되지만 새 대화에 주입되지 않으며 새로운 정보도 추출되지 않습니다.",
  ...
  "settings.memoryNoProviderBannerBody":
    "메모리 추출기에 대한 API key를 찾을 수 없습니다. LLM 기반 추출을 활성화하려면 미디어 공급자에서 OpenAI key를 추가하거나, 환경에 ANTHROPIC_API_KEY / OPENAI_API_KEY를 설정하세요. 휴리스틱 정규식 추출은 여전히 활성화되어 있습니다.",
  ...
  "updater.installerOpenBody":
    "설치 프로그램이 열려 있습니다. 업데이트를 완료할 수 있도록 Open Design을 종료하는 중입니다.",
  ...
  "updater.quitFailedBody":
    "설치 프로그램이 열려 있지만 Open Design을 종료할 수 없습니다. 앱을 교체하기 전에 Open Design을 종료하세요.",
  ...
  "settings.orbit.templateHelp":
    "skill로 Orbit을 조정합니다 — 선택한 템플릿의 예시 프롬프트가 모든 Orbit 실행에 주입되어 요약이 해당 템플릿 형식을 따릅니다.",
  ...
  "settings.orbit.runError":
    "Orbit을 실행할 수 없습니다. 로컬 daemon이 실행 중이고 커넥터가 구성되어 있는지 확인하세요.",
  ...
  "settings.orbit.gateBody":
    "Orbit은 커넥터 활동을 요약합니다. 아직 연결된 것이 없어요 — Orbit이 보고할 수 있도록 최소 하나의 통합을 추가하세요.",
  "settings.orbit.gateBodyNoKey":
    "Orbit은 커넥터 활동을 요약하며, 커넥터는 Composio를 통해 동작합니다. 커넥터 화면에서 Composio API 키를 추가해 카탈로그를 열고 첫 번째 통합을 선택하세요.",
  ...
  "settings.orbit.artifactMetaLegacy":
    "live artifact 지원이 활성화되기 전에 생성됨 — 게시하려면 Orbit을 다시 실행하세요.",
  ...
  "dsManager.emptyMine":
    "아직 디자인 시스템이 없습니다. 실제 제품 컨텍스트로 하나 만들어 초안을 검토한 뒤, 이후 프로젝트를 위해 게시하세요.",
  ...
  "dsManager.emptyTemplates":
    "아직 템플릿이 없습니다. 템플릿 게시가 활성화되면 공유를 통해 생성된 프로젝트에서 만들 수 있습니다.",
  ...
  "settings.customInstructionsDesc":
    "Open Design이 모든 채팅에서 따르는 고정 지침입니다. 저장된 메모리가 아니며, 사실, 선호 사항, 프로젝트 컨텍스트에는 메모리를 사용하세요.",
  "mcpClient.forwardedToLabel": "전달 대상:",
  "mcpClient.notForwardedToLabel": "전달되지 않는 대상:",
  "mcpClient.forwardedAcpNote":
    "stdio로 표시된 ACP 어댑터는 이 목록에서 stdio MCP 서버만 받습니다. HTTP 및 SSE 항목은 생성 시점에 제외됩니다.",
  "mcpClient.notForwardedNote":
    "해당 Agent의 경우 Agent 자체 구성 파일(예: ~/.codex/config.toml, ~/.gemini/settings.json)에서 MCP 서버를 구성하세요. 아래 서버는 거기서 자동으로 사용되지 않습니다."
};
