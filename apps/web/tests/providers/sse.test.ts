...
describe("streamViaDaemon", () => {
  ...
  it("keeps cross-agent workbench context in comprehensive mode", async () => {
    ...
    await streamViaDaemon({
      ...
      history: [
        { id: "1", role: "user", content: "把这个文件夹里的商品图片分门别类加入商品素材库" },
        {
          id: "2",
          role: "assistant",
        ...
        { id: "3", role: "user", content: "根据你生成的8个分类，把素材上传商品素材库" }
      ],
      systemPrompt: "",
      signal: new AbortController().signal,
      handlers
    ...
    const [, createRunInit] = fetchMock.mock.calls[0] as unknown as [RequestInfo | URL, RequestInit];
    const body = JSON.parse(String(createRunInit.body));
    expect(body.message).toContain("把这个文件夹里的商品图片分门别类加入商品素材库");
    expect(body.message).toContain("视觉聚类结果：白色短款牛仔裙");
    expect(body.message).toContain("根据你生成的8个分类，把素材上传商品素材库");
    expect(body.currentPrompt).toBe("根据你生成的8个分类，把素材上传商品素材库");
  });
