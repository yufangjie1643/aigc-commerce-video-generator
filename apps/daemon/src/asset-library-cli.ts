...
async function runAssetStatus(base: string, action: string | undefined, flags: Record<string, any>) {
  ...
  const [tools, embedding, videosData, qualityData] = await Promise.all([
  ...
  console.log(
    `[assets] commerceVideos=${status.commerceVideos.count} qualityVideos=${status.qualityVideos.count}`
  );
  console.log(
...
async function runAssetSearch(
  ...
  if (!query && tags.length === 0) {
    throw new Error(
      "usage: od assets search [--query <text>|--prompt-file <path|->] [--tags a,b] [--kind commerce-videos|quality-videos|all] [--granularity asset|slice|all] [--limit <n>] [--json]"
    );
  }
  const body = {
    ...(query ? { query } : {}),
...
async function runCommerceVideos(
  ...
  switch (action) {
    ...
    case "search": {
      ...
      if (!body.query)
        throw new Error(
          "usage: od assets commerce-videos search --query <text> [--connector bilibili|douyin] [--limit <n>] [--json]"
        );
    ...
    case "import-search": {
      ...
      if (!body.query)
        throw new Error(
          "usage: od assets commerce-videos import-search --query <text> [--connector bilibili|douyin] [--limit <n>] [--json]"
        );
...
function printAssetsHelp() {
  ...
  od assets search [--query <text>|--prompt-file <path|->] [--tags a,b]
                   [--kind commerce-videos|quality-videos|all]
  ...
  od assets commerce-videos search --connector <bilibili|douyin> --query <text>
  ...
  od assets commerce-videos import-search --connector <bilibili|douyin> --query <text>
                                          [--limit <n>] [--sort hot|newest|comments|favorites] [--json]
