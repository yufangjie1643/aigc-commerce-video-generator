...
export async function runCommerceVideoCli(args: string[]): Promise<{ exitCode: number }> {
  ...
  try {
    switch (command) {
      ...
      case "materials": {
        requireProject(projectId, "usage: od commerce-video materials --project <id> --materials-json <json> [--json]");
        const body = flags["materials-json"] ? parseJson(flags["materials-json"], "--materials-json") : {};
      ...
      case "storyboard": {
        requireProject(
          projectId,
          "usage: od commerce-video storyboard --project <id> --storyboard-json <json> [--json]"
        );
        const body = flags["storyboard-json"] ? parseJson(flags["storyboard-json"], "--storyboard-json") : {};
        const data = await requestJson(
          base,
      ...
      case "generate": {
        requireProject(projectId, "usage: od commerce-video generate --project <id> --model <id> [--follow] [--json]");
        ...
        if (flags.follow && data.taskId) data.job = await waitForJob(base, data.taskId, flags);
...
function printCommerceVideoHelp() {
  ...
  od commerce-video materials --project <id> --materials-json <json> [--json]
  od commerce-video script --project <id> --title <title> --hook <hook> --prompt <voiceover> [--json]
  od commerce-video storyboard --project <id> --storyboard-json <json> [--json]
  od commerce-video generate --project <id> --model <id> [--json]
  od commerce-video generate --project <id> --model <id> --follow --full-auto [--json]
