...
function printRootHelp() {
  ...
  od assets <tools|embedding|commerce-videos|quality-videos> ...
      Manage the global asset library, vectorization provider, local ffmpeg
      paths, commerce videos, and quality video reports.
...
async function runMediaUnderstand(rawArgs) {
  ...
  if (typeof flags["json-output"] === "string" && flags["json-output"].length > 0) {
    await writeCliJsonFile(flags["json-output"], data);
    return;
  }
  if (flags.json) {
    process.stdout.write(JSON.stringify(data, null, 2) + "\n");
...
async function writeCliJsonFile(filePath, data) {
...
function printMediaHelp() {
  console.log(`Usage: od media generate --surface <image|video|audio> --model <id> [opts]
       ...
       od media understand --image|--audio|--video <path|url> [--provider mimo|volcengine-ark] [--prompt <text>] [--model <id>] [--json|--json-output <path>]
...
Common options:
  ...
  --daemon-url <url>
  --json-output <path>     Write media understanding JSON directly as UTF-8.
