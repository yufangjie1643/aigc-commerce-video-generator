...
export function registerAssetLibraryRoutes(app: Express, ctx: RegisterAssetLibraryRoutesDeps): void {
  ...
  void rm(path.join(assetRoot, "products"), { recursive: true, force: true }).catch(() => undefined);
  ...
  app.post("/api/asset-library/commerce-videos/search", async (req, res) => {
    ...
    if (!["bilibili", "douyin"].includes(connectorId)) {
      return sendApiError(
        res,
        501,
        "NOT_IMPLEMENTED",
        "search preview is currently implemented for Bilibili and Douyin"
      );
    }
    ...
    const requestedToolName = cleanString(body.toolName) || `${connectorId}_search_videos`;
    ...
    try {
      ...
      res.json({
        ...
        search: {
          connectorId,
          toolName,
          provider: connectorOutput.provider ?? crawlerSearchProvider(connectorId),
          query,
          limit,
          sort,
  ...
  app.post("/api/asset-library/commerce-videos/import/search", async (req, res) => {
    ...
    if (!["bilibili", "douyin"].includes(connectorId)) {
      return sendApiError(
        ...
        501,
        "NOT_IMPLEMENTED",
        "search import is currently implemented for Bilibili and Douyin"
      );
    }
    const query = cleanString(body.query);
    if (!query) return sendApiError(res, 400, "BAD_REQUEST", "query is required");
...
const ASSET_LIBRARY_SEARCH_KINDS: AssetLibrarySection[] = ["commerce-videos", "quality-videos"];
...
function normalizeAssetLibrarySearchKinds(value: unknown): AssetLibrarySection[] {
  ...
  for (const value of values) {
    ...
    if (normalized === "product" || normalized === "products") {
      throw badAssetLibrarySearchRequest(
        "global product material search has been removed; use project commerce-video productMaterials instead"
      );
    }
...
function crawlerSearchLimitations(connectorId: string): string[] {
  ...
  if (connectorId === "douyin") {
    return [
      "抖音关键词搜索依赖已连接账号的 Cookie 和网页搜索接口，结果会受登录状态、验证码、地区与平台风控影响。",
      "仅保存搜索候选元数据，不下载原视频文件；需要素材文件时请对具体公开视频链接执行导入。",
      "互动指标以搜索接口返回字段为准，可能缺少播放量或实时数据。"
    ];
  }
  return ["仅保存连接器公开返回字段；不绕过登录、验证码或平台风控。"];
}

function crawlerSearchProvider(connectorId: string): string {
  return connectorId === "douyin" ? "douyin-web" : "bilibili-web";
}
