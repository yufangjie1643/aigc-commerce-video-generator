...
export function migrateAssetLibrary(db: Db): void {
  ...
  `);
  removeGlobalProductAssetLibrary(db);
}

function removeGlobalProductAssetLibrary(db: Db): void {
  db.exec(`
    DROP INDEX IF EXISTS idx_asset_library_products_updated;
    DROP TABLE IF EXISTS asset_library_products;
    DELETE FROM asset_library_jobs WHERE asset_kind = 'products';
  `);
  if (tableHasColumn(db, "asset_library_project_context", "product_asset_id")) {
    db.prepare(
      `DELETE FROM asset_library_project_context
        WHERE product_asset_id IS NOT NULL
          AND product_asset_id <> ''
          AND commerce_video_asset_id IS NULL
          AND slice_id IS NULL`
    ).run();
  }
}

export function insertCommerceVideoAsset(
  db: Db,
  input: {
    id: string;
    title: string;
  ...
  if (!hasCommerceVideoSourceInput(input)) {
    throw new Error("commerce video asset source data is required");
  }
...
function hasCommerceVideoSourceInput(input: {
  sourceUrl?: string | null;
  sourceVideoId?: string | null;
  filePath?: string | null;
}): boolean {
  return Boolean(input.sourceUrl?.trim() || input.sourceVideoId?.trim() || input.filePath?.trim());
}

export function listCommerceVideoAssets(
  db: Db,
...
export function getCommerceVideoAsset(db: Db, id: string): CommerceVideoAsset | null {
  const row = db.prepare(`SELECT ${VIDEO_COLS} FROM asset_library_commerce_videos WHERE id = ?`).get(id) as
    | Row
    | undefined;
...
export function getCommerceVideoAssetBySha(db: Db, sha256: string): CommerceVideoAsset | null {
  const row = db.prepare(`SELECT ${VIDEO_COLS} FROM asset_library_commerce_videos WHERE sha256 = ?`).get(sha256) as
    | Row
    | undefined;
  return row ? normalizeVideo(row) : null;
}
...
export function deleteCommerceVideoAsset(db: Db, id: string): CommerceVideoAsset | null {
  ...
  db.transaction(() => {
    ...
    db.prepare(
      "DELETE FROM asset_library_jobs WHERE asset_kind IN ('commerce-videos', 'quality-videos') AND asset_id = ?"
    ).run(id);
...
export function replaceCommerceVideoSlices(
  db: Db,
  assetId: string,
  slices: Array<{
    id: string;
...
export function insertAssetLibraryJob(
  db: Db,
  input: {
    id: string;
...
export function interruptActiveAssetLibraryJobs(db: Db): AssetLibraryJob[] {
  const active = db
    .prepare(`SELECT ${JOB_COLS} FROM asset_library_jobs WHERE status IN ('queued', 'running')`)
    .all() as Row[];
  const interrupted: AssetLibraryJob[] = [];
  for (const row of active) {
    const job = normalizeJob(row);
    const updated = updateAssetLibraryJob(db, job.id, {
      status: "interrupted",
      endedAt: Date.now(),
      error: {
        code: "DAEMON_RESTARTED",
        message: "daemon restarted before asset library job completed"
      },
      progress: [...job.progress, "interrupted: daemon restarted before asset library job completed"].slice(-200)
    });
...
export function setProjectAssetLibraryContext(
  db: Db,
  projectId: string,
  ...
  const tx = db.transaction(() => {
    ...
    const insert = db.prepare(
      `INSERT INTO asset_library_project_context
        (project_id, commerce_video_asset_id, slice_id, purpose, note, created_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    );
...
export function getProjectAssetLibraryContext(db: Db, projectId: string): ProjectAssetLibraryContextRef[] {
  const rows = db
    .prepare(
      `SELECT commerce_video_asset_id AS commerceVideoAssetId,
              slice_id AS sliceId,
              purpose,
              note
...
function normalizeVideo(row: Row): CommerceVideoAsset {
  ...
  return {
    ...
    video: {
      ...
      ...(isRecord(video.understanding) ? { understanding: video.understanding as any } : {}),
...
function normalizeJob(row: Row): AssetLibraryJob {
  ...
  return {
    id: String(row.id),
    assetKind: row.assetKind === "quality-videos" ? row.assetKind : "commerce-videos",
    assetId: String(row.assetId),
...
function normalizeContextRef(row: Row): ProjectAssetLibraryContextRef | null {
  ...
  if (typeof row.commerceVideoAssetId === "string" && row.commerceVideoAssetId.trim())
    ref.commerceVideoAssetId = row.commerceVideoAssetId.trim();
  ...
  return ref.commerceVideoAssetId || ref.sliceId ? ref : null;
}
...
function tableHasColumn(db: Db, tableName: string, columnName: string): boolean {
