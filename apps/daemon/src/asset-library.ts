...
export function insertProductAsset(
  db: Db,
  input: {
    id: string;
    title: string;
...
export function updateProductAsset(
  db: Db,
  id: string,
  patch: Partial<ProductAsset> & { product?: unknown; metadata?: unknown }
): ProductAsset | null {
...
function normalizeJob(row: Row): AssetLibraryJob {
  ...
  return {
    id: String(row.id),
    assetKind: row.assetKind === "products" || row.assetKind === "quality-videos" ? row.assetKind : "commerce-videos",
    assetId: String(row.assetId),
...
function normalizeContextRef(row: Row): ProjectAssetLibraryContextRef | null {
  ...
  if (typeof row.productAssetId === "string" && row.productAssetId.trim())
    ref.productAssetId = row.productAssetId.trim();
