...
describe("connector routes", () => {
  ...
  it("imports a product image upload into the product asset library", async () => {
    const imageBytes = Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]);
    const form = new FormData();
    form.append("file", new Blob([imageBytes], { type: "image/png" }), "blue-shirt.png");
    form.append("title", "蓝色衬衫裙正面图");
    form.append("subject", "衬衫裙");
    form.append("category", "蓝色衬衫裙");
    form.append("sellingPoints", "翻领,收腰");
    form.append(
      "metadataJson",
      JSON.stringify({ workflow: "product-image-asset-ingestion", clusterId: "blue-shirt-dress-001" })
    );

    const response = await fetch(`${baseUrl}/api/asset-library/products/import/upload`, {
      method: "POST",
      body: form
    });
    ...
    expect(body.product).toMatchObject({
      ...
      files: [
        {
          path: expect.stringMatching(/^products\//),
          name: "blue-shirt.png",
          mime: "image/png",
      ...
      metadata: {
        ...
        clusterId: "blue-shirt-dress-001",
        importedBy: "upload"
      }
    });
    expect(body.job).toMatchObject({
      kind: "process",
      assetKind: "products",
      assetId: body.product.id
    });

    const filePath = String(body.product.files?.[0]?.path ?? "");
