...
describe("composeSystemPrompt — metadata.promptTemplate", () => {
  ...
  it("adds a Codex-only built-in imagegen override for gpt-image image projects", () => {
    ...
    expect(out).toMatch(/actual\s+output path returned by the built-in imagegen result/);
    ...
    expect(out).toMatch(/report the exact source path, destination path, and access\/copy\s+error/);
    expect(out).toContain("Do not claim success, silently fall back, or ask about OpenAI/Azure");
    expect(out).toMatch(/unless the user explicitly chooses fallback in a later\s+turn/);
  ...
  it("keeps non-Codex image projects on the daemon media dispatcher contract", () => {
    ...
    expect(out).toContain('"$OD_NODE_BIN" "$OD_BIN" media generate --surface image --model <imageModel>');
  ...
  it("surfaces ElevenLabs voice options for project discovery when no voice was preselected", () => {
    const voiceOptions = Array.from({ length: 50 }, (_, index) => {
      ...
      return {
        ...
        voiceId: ordinal === 1 ? "21m00Tcm4TlvDq8ikWAM" : ordinal === 2 ? "pNInz6obpgDQGcFmaJgB" : `voice-${ordinal}`,
        category: "premade",
        labels:
          ordinal === 1
