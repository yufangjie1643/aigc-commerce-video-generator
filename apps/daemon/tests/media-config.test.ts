...
const OPENAI_ENV_KEYS = ["OD_OPENAI_API_KEY", "OPENAI_API_KEY", "AZURE_API_KEY", "AZURE_OPENAI_API_KEY"];

describe("media-config OpenAI auth-file fallback", () => {
  let homeDir: string;
  let projectRoot: string;
  const originalHome = process.env.HOME;
  const originalEnv = Object.fromEntries(OPENAI_ENV_KEYS.map((key) => [key, process.env[key]]));
  ...
  it("keeps Volcengine generation env keys separate from generic Ark env keys", async () => {
    const keys = [
      "OD_VOLCENGINE_API_KEY",
      "VOLCENGINE_API_KEY",
      "OD_VOLCENGINE_ARK_API_KEY",
      "ARK_API_KEY",
      "VOLCENGINE_ARK_API_KEY"
    ];
    const original = Object.fromEntries(keys.map((key) => [key, process.env[key]]));
    try {
      process.env.OD_VOLCENGINE_API_KEY = "generation-env-key";
      process.env.ARK_API_KEY = "generic-ark-env-key";
      ...
      await expect(resolveProviderConfig(projectRoot, "volcengine")).resolves.toMatchObject({
        apiKey: "generation-env-key"
      });
      await expect(resolveProviderConfig(projectRoot, "volcengine-ark")).resolves.toMatchObject({
        apiKey: "generic-ark-env-key"
      });
    } finally {
  ...
  describe("OD_MEDIA_CONFIG_DIR / OD_DATA_DIR storage routing", () => {
    ...
    async function writeProvidersAt(dir: string, data: unknown) {
      ...
      await writeFile(path.join(dir, "media-config.json"), JSON.stringify(data), "utf8");
    }
    ...
    it("writeConfig creates the override directory tree on first write", async () => {
      ...
      const onDisk = await readFile(path.join(target, "media-config.json"), "utf8");
...
describe("media-config Grok / xAI OAuth fallback", () => {
  ...
  let projectRoot: string;
  const originalHome = process.env.HOME;
  const originalEnv = Object.fromEntries(GROK_ENV_KEYS.map((key) => [key, process.env[key]]));
  const originalMediaConfigDir = process.env.OD_MEDIA_CONFIG_DIR;
  const originalDataDir = process.env.OD_DATA_DIR;
  let homedirSpy: ReturnType<typeof vi.spyOn>;
  ...
  async function writeOdXaiTokens(token: { accessToken: string; refreshToken?: string; expiresAt?: number }) {
...
describe("media-config model alias resolution (issue #1277)", () => {
  ...
  it("redirects via the stored aliases map in media-config.json", async () => {
    ...
    expect(await resolveModelAlias(projectRoot, "doubao-seedream-3-0-t2i-250415")).toBe("doubao-seedream-5-0");
  });
  ...
  it("lets the env var override an on-disk alias (env wins for power users)", async () => {
    ...
    expect(await resolveModelAlias(projectRoot, "doubao-seedream-3-0-t2i-250415")).toBe("env-alias");
  });
  ...
  it("writeConfig preserves aliases when a Settings-style provider PUT lands", async () => {
    ...
    const onDisk = JSON.parse(await readFile(path.join(projectRoot, ".od", "media-config.json"), "utf8"));
...
describe("seedProviderIfMissing", () => {
  ...
  const originalEnv = Object.fromEntries(SENSEAUDIO_ENV_KEYS.map((key) => [key, process.env[key]]));
  const originalMediaConfigDir = process.env.OD_MEDIA_CONFIG_DIR;
  const originalDataDir = process.env.OD_DATA_DIR;
