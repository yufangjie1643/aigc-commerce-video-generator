...
describe("od assets CLI", () => {
  ...
  beforeEach(() => {
    ...
    fetchMock = vi.fn(async (input: unknown) => {
      ...
      if (url.endsWith("/api/asset-library/products")) {
        return jsonResponse({
          products: [
            { id: "prod-1", title: "白色短款牛仔裙-1", status: "ready", subject: "牛仔裙", category: "白色短款牛仔裙" },
            {
              id: "prod-2",
              title: "蓝色中长款牛仔半身裙-1",
              status: "processing",
  ...
  it("reports aggregate asset-library status as JSON", async () => {
    ...
    expect(body.products.recent[0]).toMatchObject({
      id: "prod-1",
      category: "白色短款牛仔裙"
    });
    expect(fetchMock.mock.calls.map((call) => String(call[0]))).toEqual([
      ...
      "http://127.0.0.1:17456/api/asset-library/products",
